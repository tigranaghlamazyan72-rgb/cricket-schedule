...
const TEAM = [
  { name: "Анна",      password: "anna123"   },
  { name: "Борис",     password: "boris123"  },
  { name: "Виктория",  password: "vika123"   },
  { name: "Григорий",  password: "grisha123" },
  { name: "Диана",     password: "diana123"  },
  { name: "Евгений",   password: "zhenya123" },
  { name: "Жанна",     password: "zhanna123" },
];
const NIGHT = { name: "Захар" };
const ADMIN  = { name: "admin", password: "cricket2024" };
...
const SHIFTS = [
  ...
  { id:"N", label:"01:00 – 09:00", short:"01–09", emoji:"🌙", color:"#38bdf8", accent:"#0284c7", bg:"rgba(56,189,248,0.08)"  },
];
...
const STORAGE_KEY = "cricket_sched_v2";
...
function buildSchedule(requests) {
  ...
  DAYS.forEach(d => { s[d.key]["N"].push(NIGHT.name); });
...
export default function App() {
  ...
  function handleSave() {
    setRequests(prev => ({ ...prev, [user.name]: draft }));
    setSaved(true);
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080d14", fontFamily:"'DM Sans','Segoe UI',sans-serif", color:"#e2e8f0", display:"flex", flexDirection:"column" }}>
      ...
      <main style={{ flex:1, maxWidth:880, width:"100%", margin:"0 auto", padding:"32px 20px 60px" }}>
        ...
        {page==="pick" && user && !user.isAdmin && (
          <div className="fade">
            <div style={{ marginBottom:28 }}>
              <h2 style={{ fontSize:24, fontWeight:800, letterSpacing:"-0.5px", marginBottom:6 }}>Привет, {user.name}! 👋</h2>
              <p style={{ color:"#64748b", fontSize:14 }}>Выбери желаемые смены — по одной на каждый день</p>
            </div>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:24 }}>
              {SHIFTS.filter(s=>s.id!=="N").map(s => (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {DAYS.map(d => {
                ...
                return (
                  <div key={d.key} style={{ background:d.isToday?"rgba(34,197,94,0.05)":d.isWeekend?"rgba(255,255,255,0.015)":"rgba(255,255,255,0.025)", border:`1px solid ${d.isToday?"rgba(34,197,94,0.25)":"rgba(255,255,255,0.06)"}`, borderRadius:14, padding:"14px 18px", display:"flex", alignItems:"center", gap:12, flexWrap:"wrap", boxShadow:d.isToday?"0 0 0 2px rgba(34,197,94,0.2)":"none" }}>
                    <div style={{ minWidth:60 }}>
                      <div style={{ fontWeight:800, fontSize:15, color:d.isToday?"#4ade80":d.isWeekend?"#94a3b8":"#e2e8f0", fontFamily:"'DM Mono',monospace" }}>{d.label}</div>
                    ...
                    <div style={{ display:"flex", gap:6, flex:1, flexWrap:"wrap", justifyContent:"flex-end" }}>
                      {SHIFTS.filter(s=>s.id!=="N").map(shift => {
            ...
            <div style={{ marginTop:28, display:"flex", gap:14, alignItems:"center", justifyContent:"flex-end" }}>
              ...
              <button className="rip" onClick={handleSave} style={{ padding:"14px 36px", background:"linear-gradient(135deg,#22c55e,#15803d)", borderRadius:14, color:"white", fontSize:16, fontWeight:800, boxShadow:"0 4px 24px rgba(34,197,94,0.3)" }}>💾 Подать заявку</button>
            </div>
          </div>
        )}
        ...
        {page==="schedule" && (
          <div className="fade">
                      ...
                      {DAYS.map(d => {
                        ...
                        return (
                          <td key={d.key} style={{ padding:"10px 8px", textAlign:"center", background:d.isToday?"rgba(34,197,94,0.03)":"transparent", verticalAlign:"middle", borderRight:"1px solid rgba(255,255,255,0.03)" }}>
                            {people.length===0 ? <span style={{ color:"#1e293b", fontSize:20 }}>·</span> : (
                              <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                                {people.map(p => <div key={p} style={{ background:shift.bg, border:`1px solid ${shift.accent}44`, borderRadius:8, padding:"4px 8px", fontSize:11, fontWeight:700, color:shift.color, whiteSpace:"nowrap" }}>{p}</div>)}
                              </div>
                            )}
                          </td>
                        );
            ...
            {TEAM.filter(m=>!submitted.find(s=>s.name===m.name)).length>0 && (
              <div style={{ marginTop:20, background:"rgba(239,68,68,0.05)", border:"1px solid rgba(239,68,68,0.15)", borderRadius:14, padding:"14px 18px", display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
                <span style={{ fontSize:13, color:"#f87171", fontWeight:700 }}>⏳ Ещё не подали заявку:</span>
                {TEAM.filter(m=>!submitted.find(s=>s.name===m.name)).map(m => <span key={m.name} style={{ background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", color:"#fca5a5", borderRadius:8, padding:"3px 10px", fontSize:12, fontWeight:700 }}>{m.name}</span>)}
              </div>
            )}
            <div style={{ marginTop:16, background:"rgba(56,189,248,0.05)", border:"1px solid rgba(56,189,248,0.15)", borderRadius:14, padding:"14px 18px", display:"flex", alignItems:"center", gap:12 }}>
              <span style={{ fontSize:24 }}>🌙</span>
              <div>
                <div style={{ fontWeight:700, color:"#38bdf8", fontSize:14 }}>{NIGHT.name} — фиксированная ночная смена</div>
                <div style={{ fontSize:12, color:"#475569", marginTop:2 }}>01:00 – 09:00 · каждый день</div>
              </div>
            </div>
          </div>
        )}
